...
- [ ] **Step 1: 先写 DemoSource 的失败测试，锁定示例来源输出与 limit 截断行为**
...
- [ ] **Step 2: 再写一个集成失败测试，锁定多来源合并入口先按来源顺序串联结果**
...
- [ ] **Step 3: 运行测试，确认 DemoSource 和多来源调度入口尚未实现时失败**
...
- [ ] **Step 4: 写最小 DemoSource 实现，提供稳定的无网络候选输出**
...
- [ ] **Step 5: 在 CLI 脚本中加入来源注册表与多来源候选收集入口**
...
- [ ] **Step 6: 运行测试，确认 DemoSource 与多来源调度入口通过**
...
- [ ] **Step 3: 运行测试，确认 storage 尚未实现时失败**
...
- [ ] **Step 4: 写最小 storage 实现，完成 hash、索引、历史去重与元数据写入**
...
- [ ] **Step 5: 在 CLI 脚本中补一个最小存储入口，供后续主流程复用**
...
- [ ] **Step 6: 再次运行测试，确认 storage 与历史索引通过**
...
- [ ] **Step 1: 先写失败测试，锁定报告文本必须包含来源贡献、去重数量和保存目录**
...
- [ ] **Step 2: 再写一个 CLI 失败测试，锁定新主流程会调用多来源收集、去重和报告输出**
...
- [ ] **Step 3: 运行测试，确认 reporting 和新 CLI 流程尚未实现时失败**
...
- [ ] **Step 4: 写最小 reporting 实现，输出可解释的结果摘要**
...
- [ ] **Step 5: 改写 CLI 主流程，让它接入多来源、去重、历史索引与报告输出**
